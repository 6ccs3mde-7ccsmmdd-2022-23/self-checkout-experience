...
class SelfCheckoutExperienceScopeProvider extends AbstractDeclarativeScopeProvider {
	...
	dispatch def IScope visibleVariablesScope(Self_checkout tp) {
		scopeFor(tp.selfCheckoutExperience.filter(VariableDeclaration))
	}
	
	dispatch def IScope visibleVariablesScope(Repeat ls) {
		ls.eContainer.internalVisibleVariablesScope
	}

	dispatch def IScope internalVisibleVariablesScope(Self_checkout tp) {
		scopeFor(tp.selfCheckoutExperience.filter(VariableDeclaration))
	}
